...
function Paragraph({ id }) {
  ...
  useEffect(() => {
    ...
    if (typeof accumulatedMessages !== undefined
        && Array.isArray(accumulatedMessages) && accumulatedMessages.length > 0) {
  ...
  const handleOrdinalChange = (e) => {
    ...
    const payload = [{ ...paragraph, ordinal }];
  ...
  const isLockedByLocalAuthor = () => paragraph.lockedBy !== undefined
      && paragraph.lockedBy !== null
      && paragraph.lockedBy.id === author.id;
  ...
  const isLocked = () => !(paragraph.lockedBy === undefined || paragraph.lockedBy === null);
  ...
  return (
    <div tabIndex={paragraph.ordinal} className={`paragraph divider-color ${isLockedByLocalAuthor() ? 'editing' : 'locked'}`} onFocus={handleClickInside} onBlur={handleClickOutside}>
      <div className="paragraphHeader">
        ...
        <div>
          <input
            value={paragraph.ordinal}
            type="number"
            disabled={error}
            readOnly={isLocked() && !isLockedByLocalAuthor()}
